package com.mtw.alejandro.booksqueryapp
...
import com.mtw.alejandro.booksqueryapp.api.BooksApiService
import com.mtw.alejandro.booksqueryapp.api.BooksModel
...
class MainActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener, BooksAdapter.OnItemClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        search_view.setOnQueryTextListener(object : MaterialSearchView.OnQueryTextListener {
            override fun onQueryTextSubmit(query: String): Boolean {
                if (!isOnline()){
                    showAlert("Error","No hay conexión con la red")
                } else {
                    searchBooks(query)
                }
                return false
    ...
    private  fun searchBooks(query: String){
        //Ejecutar la llamada a nuestra API. Ejecutar el metodo searchBooks
        disposable = booksApiService.searchBooks(query)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(
                        {result ->
                            // Mostrar la lista de libros regresados
                            viewAdapter.setBooks(result.items)
                            Toast.makeText(this@MainActivity,"Busqueda realizada:" + result.items.size +" libros recuperados",Toast.LENGTH_LONG).show()
                        },
                        {error ->
